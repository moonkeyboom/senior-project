...
1. **Ω1 = 1 − |Θ−θ|/Θ** ⚠️ **แก้แล้ว (2026-08-01)** — เดิมโค้ดใช้ `θ/Θ` ซึ่ง**ผิด**
   - ดึงข้อความจริงจาก paper หน้า 8 ได้ worked examples ตรงๆ:
     `A1: (1 − |2−1|/2) × (10−0)/(10−0) × 1/(1+5.07) = 0.08`
     `A2: (1 − |2−2|/2) × (6−0)/(7−0) × 1/(1+2.89) = 0.22`
   - **ทำไมไม่มีใครจับได้**: บน Table 3 ทั้ง A1 (θ=1,Θ=2) และ A2 (θ=2,Θ=2) มี θ ≤ Θ
     ซึ่งเป็นช่วงที่สองสูตรให้ค่า**เท่ากันเป๊ะ** → `test_paper_examples()` ผ่านทั้งที่สูตรผิด
   - **พังตอน θ > Θ**: สูตรถูกต้องต้องลดลง แต่ `θ/Θ` ไป clamp ติดเพดาน 1.0
     เช่น θ=3,Θ=2 → ถูก 0.5 / ผิด 1.0 ; θ=6,Θ=1 → ถูก 0.0 / ผิด 1.0
   - semantic: Θ = จำนวนเกรดที่ **"ought to remain unassigned"** (paper บรรทัด 399-400)
     ดังนั้นเป็น penalty **สองทาง** — เว้นน้อยไปหรือมากไปก็โดนหักทั้งคู่
...
4b. **WGF-CPD reduce-from-|L| (Algorithm 1) — implement แล้ว** `wgf_reduce()` ใน
   `optimal_cpd_omega_prime.py` เริ่มจาก |L| เกรด แล้วตัดทีละตัวขณะที่ Θ ≥ 1
   - **กฎการตัด label**: ตัด label ที่อยู่**ใต้ gap ที่เข้าเงื่อนไข** ถ้า gap นั้นคือ γl
     (อยู่ใต้ label ต่ำสุด) → ตัด label ต่ำสุดทิ้ง แล้วคะแนนที่กำพร้ายุบขึ้นไปรวมกับ label ที่เหลือ
   - verify กับ `CPDtemplateV2.xlsx` (ทำมือ 139 คะแนน, |L|=8): iter1 N=8 θ=0 Θ=1 Ω1=0
     → ตัด F → iter2 N=7 θ=1 Θ=1 Ω1=1.0, D กลายเป็น [39.5..35] PVI=4.5 **ตรงเป๊ะ**
   - **เงื่อนไขหยุด: วนจน Θ = 0** (Requirement 1 เป็น *constraint* ไม่ใช่คะแนนที่จะ maximize)
     ผลลัพธ์ที่คืนคือ state สุดท้ายที่ Θ=0 **ไม่ใช่** iteration ที่ Ω′ สูงสุด
     บนข้อมูลจริงนี้: N=8→7→6→5→4→**3** (Θ=0, Ω′=0.0383) ทั้งที่ N=7 มี Ω′=0.1168 สูงกว่า
   - หมายเหตุ: ค่า 0.125 ในชีตเป็น *unconditional* metric คนละตัวกับ Ω′ Eq.(2) — อย่าเอามาเทียบ
