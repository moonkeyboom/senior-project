...
def ckwrap_1d(x: np.ndarray, k: int):
    ...
    raw_labels = np.asarray(result.labels, dtype=int)

    # Do NOT assume raw_labels are already 0..k-1 contiguous ascending-by-
    # value (this varies across ckwrap versions). Re-map explicitly by
    # sorting the unique label values according to their mean x, so the
    # contract "label 0 = lowest-value cluster" always holds.
    unique_raw = np.unique(raw_labels)
    raw_means = np.array([x[raw_labels == u].mean() for u in unique_raw])
    order = np.argsort(raw_means)
    remap = {int(unique_raw[old]): new for new, old in enumerate(order)}
    labels = np.array([remap[l] for l in raw_labels], dtype=int)

    # Compute centers and withinss directly from actual cluster members,
    # rather than trusting result.centers, so this is robust regardless
    # of how ckwrap orders/reports its center array.
    centers = np.empty(k, dtype=float)
    withinss = 0.0
    for c in range(k):
        pts = x[labels == c]
