...
# ============================================================================
# CPD REFINEMENT ALGORITHM - DETAILED EXECUTION
# ============================================================================
...
# ============================================================================
# CPD HELPER FUNCTIONS
# ============================================================================

def compute_score_gaps(sorted_scores):
    '''Computes score gaps between consecutive sorted scores'''
...
def determine_label_to_remove(gap_index, sorted_scores, grade_symbols, grades_df):
    '''Determines which grade label to remove based on the gap position'''
    score_above = sorted_scores[gap_index]
    score_below = sorted_scores[gap_index + 1]

    grade_above = grades_df[grades_df['Score'] == score_above]['Grade'].values[0]
    grade_below = grades_df[grades_df['Score'] == score_below]['Grade'].values[0]

    idx_above = grade_symbols.index(grade_above)
    idx_below = grade_symbols.index(grade_below)

    # Special case: if gap is at lower bound (last gap)
    if gap_index == len(sorted_scores) - 2:
        return grade_above
    else:
        return grade_below
...
# ============================================================================
# MAIN CPD REFINEMENT LOOP
# ============================================================================

def cpd_refinement_loop_detailed(scores, initial_grades_df, grade_symbols, max_iterations=10):
    '''CPD refinement loop with detailed logging'''
    ...
    print('\n' + '='*80)
    print('CPD REFINEMENT LOOP - DETAILED EXECUTION LOG')
    print('='*80)
    ...
    while iteration < max_iterations:
        ...
        # Step 2: Compute gaps and PVIs
        gaps = compute_score_gaps(scores)
        ...
        for idx, gap_idx in enumerate(max_gap_indices, 1):
            ...
            label_to_remove = determine_label_to_remove(
                gap_idx, scores, current_grade_symbols, current_grades_df
            )
        ...
        current_grades_df = best_candidate['grades_df']
...
if empty_grades:
    ...
    print('No empty grades')

print(f"Label count: {len(cpd_result['final_grade_symbols'])} (started with {len(grade_symbols)})")
print(f"Final Omega computed correctly: {cpd_result['final_omega']:.4f}")

# Save final results
output_df = cpd_result['final_grades_df'].copy()
output_file = './file/output/cpd_refined_result.xlsx'
